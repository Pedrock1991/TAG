	...
	void CalculatePassengersMovement (Vector3 velocity) {
		...
		if (velocity.y != 0) {
			...
			for (int i = 0; i < verticalRayCount; i++) {
				...
				RaycastHit2D hit = Physics2D.Raycast (rayOrigin, Vector2.up * directionY, rayLenght, passagerMask);
		...
		if (velocity.x != 0) {
			...
			for (int i = 0; i < horizontalRayCount; i++) {
				...
				RaycastHit2D hit = Physics2D.Raycast (rayOrigin, Vector2.right * directionX, rayLenght, passagerMask);
		...
		//Passager on top of a horizontally or downward moving platform
		if (directionY == -1 || velocity.y == 0 && velocity.x != 0) {
			float rayLenght = skinWidth * 2;

			for (int i = 0; i < verticalRayCount; i++) {
				Vector2 rayOrigin = raycastOrigins.topLeft+ Vector2.right * (verticalRaySpacing * i);
				RaycastHit2D hit = Physics2D.Raycast (rayOrigin, Vector2.up, rayLenght, passagerMask);
