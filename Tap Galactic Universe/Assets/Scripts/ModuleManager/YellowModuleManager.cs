...
public class YellowModuleManager : MonoBehaviour {
	...
	public YellowClick yellowClick;
	SaveModule save;
	...
	void Update () {
		...
		moduleBonus.text = "Bonus\n\t+ " + formatter.FormatNumber( CalculateBonus ()) + " Parsec";
		...
		if (greenClick.data >= cost) {
			...
			case 1:
				...
				for (i = 1; i < (levelScale + 1); i++) {
					...
					yellowClick.parsecPerProbe += bonusScale;
					bonus += bonusScale;
					level++;
					manager.knowledge++;
				}
