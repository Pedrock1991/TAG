...
public class BlueModuleManager : MonoBehaviour {
	...
	void Update () {
		...
		moduleBonus.text = "Bonus\n\t+ " + formatter.FormatNumber( CalculateBonus ()) + " Scanning Power";
		...
		if (greenClick.data >= cost) {
			...
			case 1:
				...
				for (i = 1; i < (levelScale + 1); i++) {
					...
					blueClick.scanningPerProbe += bonusScale;
					bonus += bonusScale;
					level++;
					manager.knowledge++;
				}
	...
	public void SaveGame () {
		...
		PlayerPrefs.SetString ("BlueModuleManagerSave" + modName, Helper.Serialize<SaveModule> (save));
	}

	public void LoadGame () {
		if (PlayerPrefs.HasKey ("BlueModuleManagerSave" + modName)) {
			save = Helper.DeSerialize<SaveModule>(PlayerPrefs.GetString("BlueModuleManagerSave" + modName));
