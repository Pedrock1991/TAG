...
public class RedModuleManager : MonoBehaviour {
	...
	void Update () {
		...
		moduleBonus.text = "Bonus\n\t+ " + formatter.FormatNumber( CalculateBonus ()) + " Damage";
		...
		if (greenClick.data >= cost) {
			...
			case 1:
				...
				for (i = 1; i < (levelScale + 1); i++) {
					...
					redClick.damagePerProbe += bonusScale;
					bonus += bonusScale;
					level++;
					manager.knowledge++;
				}
	...
	public void SaveGame () {
		...
		PlayerPrefs.SetString ("RedModuleManagerSave" + modName, Helper.Serialize<SaveModule> (save));
	}

	public void LoadGame () {
		if (PlayerPrefs.HasKey ("RedModuleManagerSave" + modName)) {
			save = Helper.DeSerialize<SaveModule>(PlayerPrefs.GetString("RedModuleManagerSave" + modName));
